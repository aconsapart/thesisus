...
class ResearchState(TypedDict):
    ...
    discovered_strategy_log: list[str]
...
The obstruction is the exact short-box product-fiber curve-intersection theorem for the
trace-zero Möbius involution family
...
is too strong in arbitrary tiny boxes, and the shifted-product multiset formula using
only B=2(p+1)(q+1) is an overcount unless the A-constraint is retained.

The corrected frontier is the exact product-fiber incidence problem:
...
Known useful facts:
    ...
    PROVED: the first-pair exact curve is βpq+2α(p+q)+δ=0.
    PROVED: if β!=0, (βp+2α)(βq+2α)=6Q(R) mod ell.
    PROVED/CORRECTED: using only w(B) is an overcount; exact fibers require I_{P,Q}(A,B).
    OPEN: random-sized second moment for exact short-box product-fiber curve intersections.

Hybrid strategy currently considered strongest:

    - integer hyperbola / divisor bounds for lifted or small-Q(R) fibers,
    - finite-field shifted-product / character methods for large non-lifted fibers,
    - exact (A,B)-incidence to remove false positives,
    - near-injectivity and divisor-window rigidity as fallback for remaining small-volume fibers.
"""
...
def default_strategies() -> list[StrategyState]:
    raw = [
        (
            ...
            "Hybrid exact product-fiber attack",
            "Combine integer hyperbola/divisor bounds, finite-field shifted-product methods, exact (A,B)-curve incidence, near-injectivity, and geometric divisor-window rigidity. First split by lifted vs non-lifted fibers, Q(R)=0 vs Q(R)!=0, beta=0, alpha=0, s=0, and exact A-constraints. This is the primary combined strategy lane.",
            -4,
        ),
        (
            "meta_strategy_discovery",
            "Meta-strategy discovery and falsification",
            "Actively search for new proof strategies, hidden equivalences, and falsification tests. Propose new StrategyState entries only when they are materially different from existing strategies and include their first falsification test.",
            -3,
        ),
        (
            "exact_ab_curve_incidence",
            "Exact (A,B)-curve incidence",
            "Attack the corrected exact product-fiber theorem using I_{P,Q}(A,B), not only w(B). Count intersections of rational product-fiber curves with the exact image (p,q)->(A,B).",
            -2,
        ),
        (
...
def _slugify_strategy_id(text: str) -> str:
    text = text.strip().lower()
    text = re.sub(r"[^a-z0-9]+", "_", text)
    text = text.strip("_")
    return text[:64] or "discovered_strategy"


def node_discover_strategies(state: ResearchState) -> dict[str, Any]:
    """Optionally add new strategies discovered by the meta-strategy lane.

    This node is deliberately conservative: it only appends strategies whose IDs are new,
    and it asks for a first falsification test. The goal is to keep the portfolio from
    recycling already-demoted strategies while still allowing genuinely new attacks.
    """
    if state["resolved"]:
        return {}

    prompt = f"""
You are updating the strategy portfolio for the Erdős proof-search automation.

Current frontier:
{state['current_frontier']}

Latest synthesis:
{state['last_synthesis'][:12000]}

Existing strategies:
{json.dumps([{ 'id': s['id'], 'title': s['title'], 'status': s['status'], 'score': s['score']} for s in state['strategies']], indent=2)[:12000]}

Task:
Propose 0 to 3 genuinely new strategies only if they are materially different from existing strategies.
Each proposed strategy must include a falsification test that would kill or sharply revise it.
Do not propose generic 'try everything' strategies.

Return JSON list only, each item with:
- id
- title
- description
- priority integer, where lower means more urgent
- first_falsification_test
"""
    raw = call_llm(prompt)
    write_artifact(state, f"iteration_{state['iteration']:02d}/strategy_discovery_raw.json", raw)
    proposals = parse_json_list(raw)

    existing = {s["id"] for s in state["strategies"]}
    strategies = list(state["strategies"])
    log = list(state.get("discovered_strategy_log", []))
    added_lines: list[str] = []

    for item in proposals[:3]:
        sid = _slugify_strategy_id(str(item.get("id") or item.get("title") or "discovered_strategy"))
        if sid in existing:
            continue
        title = str(item.get("title") or sid.replace("_", " ").title())
        desc = str(item.get("description") or "Discovered strategy")
        falsify = str(item.get("first_falsification_test") or "No falsification test provided.")
        if falsify.strip() == "" or "generic" in desc.lower() and "specific" not in desc.lower():
            continue
        try:
        ...
        strategy = StrategyState(
            id=sid,
            title=title,
            description=desc + "\n\nFirst falsification test: " + falsify,
            priority=priority,
            score=max(25.0, 90.0 - 5.0 * priority),
            status="active",
            last_result="",
            falsifications=[],
            proved=[],
        ...
        existing.add(sid)
        added_lines.append(f"- {sid}: {title}\n  Falsification: {falsify}")

    if added_lines:
        report = "# New discovered strategies\n\n" + "\n".join(added_lines)
    else:
        report = "# New discovered strategies\n\nNo new materially distinct strategies added."
    write_artifact(state, f"iteration_{state['iteration']:02d}/strategy_discovery_report.md", report)
    log.append(report)
    strategies.sort(key=lambda s: int(s["priority"]))
    return {"strategies": strategies, "discovered_strategy_log": log}
...
def build_graph():
    ...
    graph.add_node("discover_strategies", node_discover_strategies)
    ...
    graph.add_edge("synthesize", "discover_strategies")
    graph.add_conditional_edges("discover_strategies", should_continue, {"continue": "increment", "end": END})
