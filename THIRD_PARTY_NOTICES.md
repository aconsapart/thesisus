...
## The AI Scientist (SakanaAI/AI-Scientist)

The paper pipeline under `src/thesius/paper/` is adapted from
[SakanaAI/AI-Scientist](https://github.com/SakanaAI/AI-Scientist):

- `src/thesius/paper/llm.py` — adapted from `ai_scientist/llm.py`
- `src/thesius/paper/writeup.py` — adapted from `ai_scientist/perform_writeup.py`
- `src/thesius/paper/review.py` — adapted from `ai_scientist/perform_review.py`
- `src/thesius/paper/latex/` — adapted from `templates/*/latex/`
- `src/thesius/paper/fewshot_examples/` — copied from `ai_scientist/fewshot_examples/`

These files are governed by **The AI Scientist Source Code License, Version
1.0** (Sakana AI), reproduced in full below as required by its Section 3.1.
Note in particular:

- The Section 3.2 use restrictions (including the requirement to prominently
  disclose that generated manuscripts are machine-generated) apply to these
  files and to papers produced with them, and per Section 3.3 must be carried
